...
function SWEP:PrimaryAttack()
	...
	self.Weapon:EmitSound("/radio/axis/artillerybegin"..(math.random(1,5))..".ogg")
	self.Weapon:SendWeaponAnim(ACT_VM_PRIMARYATTACK)
	self:TakePrimaryAmmo(1)
	
	if CLIENT or game.SinglePlayer() then self.Owner:ChatPrint("[GREDWITCH'S SWEPS]Nebelwerfer strike begins in "..(self.strikedalay).." seconds") end
	if SERVER then
		...
		if traceRes.Hit then
			...
			spawnpos = hitpos + Vector(0,0,1000)
		end
