...
SWEP.PrintName				= "Nebelwerfer Artillery Support Marker"
...
if  ( !self:CanPrimaryAttack() ) then return end
	...
	self.Weapon:EmitSound("/radio/axis/artillerybegin"..(math.random(1,4))..".ogg")
	self.Weapon:SendWeaponAnim(ACT_VM_PRIMARYATTACK)
	self:TakePrimaryAmmo(1)
	self.Owner:ChatPrint("Artillery Strike begins in "..(self.strikedalay).." seconds")
	...
	if SERVER then
		...
		--[[for i = 1, 12 do
				timer.Simple((math.random(0.5,1)), function()
					--self.Rocket:EmitSound("artillery/nebelwerfer/distant_rocket_artillery_fire_0"..(math.random(1,4))..".wav", 0, 100, 1)
					if traceRes.Hit then
						spawnpos = traceRes.HitPos - Vector(math.random(-1000,1000),math.random(-1000,1000),64)
					else
						spawnpos = hitpos + Vector(math.random(-1000,1000),math.random(-1000,1000),1000)
		...
		timer.Simple((math.random(0.5,1)), function() self.Rocket:EmitSound("artillery/nebelwerfer/distant_rocket_artillery_fire_0"..(math.random(1,4))..".wav", 0, 100, 1) end)
		self.Rocket = ents.Create(self.strikeentity)
		self.Rocket:SetPos(spawnpos)
		self.Rocket:SetAngles(Angle(90,0,0))
		self.Rocket:Spawn()
