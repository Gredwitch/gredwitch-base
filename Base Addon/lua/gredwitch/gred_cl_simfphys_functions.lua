			...
			if v and v[Mode] then
				...
				if IsValid(seat) then
					if v[Mode].Sight then
						seat.SightAttachment = vehicle:LookupAttachment(SeatTab.Sight.SightAttachment)
					end
...
gred.TankHandleTracks = function(vehicle,TrackVector,TracksTab,SusDataTab)
	if TracksTab then
		...
		local speed = math.Clamp(vel * 0.0001,0,TracksTab.TrackSoundVolume)
...
gred.TankHandleSeats = function(vehicle,Mode,ply,VehicleSeatTab,ViewPortsTab)
	if vehicle.LocalPlayerActiveSeat then
		...
		if SeatTab then
			if (vehicle.LocalPlayerActiveSeat.MuzzleAttachment or SeatTab.IsLoader or SeatTab.Hatches or SeatTab.IsCommander) and !ply.HookAdded then
