...
local function cannon_fire(ply,vehicle,shootOrigin,shootDirection)
	...
	local tAttachment = vehicle:GetAttachment( vehicle:LookupAttachment( "turret_cannon" ) )
	local shell = ents.Create ("gb_rocket_105mm")
	shell:SetPos(shootOrigin)
	shell:SetAngles(tAttachment.Ang:Up())
	shell:Spawn()
	shell:Activate()
	shell:Launch()
end
