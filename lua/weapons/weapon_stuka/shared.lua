SWEP.strikedalay        = 5
SWEP.strikeentity       = "gb_bomb_sc500"
...
SWEP.PrintName				= "Stuka dive bomb marker"
...
if  ( !self:CanPrimaryAttack() ) then return end
	...
	self.Weapon:EmitSound("radio/axis/stukadivebegin"..(math.random(1,5))..".ogg")
	...
	if SERVER then
		...
		if traceRes.Hit then
			spawnpos = traceRes.HitPos - Vector(0,0,700)
		else
			spawnpos = hitpos + Vector(0,0,700)
		end
		
		timer.Simple( (self.strikedalay), function() 
		   self.Owner:ChatPrint("Air Strike is launched!") 	
		self.Weapon:EmitSound("artillery/flyby/stuka_dive_bomb.ogg", 0, 100, 1, CHAN_AUTO)
		...
		self.Rocket:Arm()
		self.RocketPhysObj = self.Rocket:GetPhysicsObject() end)
